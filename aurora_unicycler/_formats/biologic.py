...
def to_biologic_mps(
    ...
    default_step = {
        ...
        "rec2_value": "",
        "rec2_value_unit": "",
        "E range min (V)": f"{min(range_V):.3f}",
        "E range max (V)": f"{max(range_V):.3f}",
        "I Range": "Auto",
        "I Range min": "Unset",
        "I Range max": "Unset",
        "I Range init": "Unset",
...
def _generate_safety(protocol: _core.BaseProtocol, range_V: tuple[float, float]) -> list[str]:
    """Generate safety settings as list of strings."""
...
def _generate_ocv(step: _core.OpenCircuitVoltage) -> dict[str, str]:
    return {
        "ctrl_type": "Rest",
        "lim_nb": "1",
        "lim1_type": "Time",
...
def _generate_cc(step: _core.ConstantCurrent, protocol: _core.BaseProtocol) -> dict[str, str]:
    """Generate constant current dictionary."""
    step_dict = {}
...
def _generate_cv(
    step: _core.ConstantVoltage, protocol: _core.BaseProtocol, step_number: int
) -> dict[str, str]:
    """Generate constant voltage dictionary."""
    step_dict = {}
    step_dict.update(
        {
            "ctrl_type": "CV",
    ...
    if step_number > 0:
        prev_mA = None
        prev_step = protocol.method[step_number - 1]
...
def _generate_eis(step: _core.ImpedanceSpectroscopy) -> dict[str, str]:
    """Generate EIS dictionary."""
    step_dict = {}
    if step.amplitude_V:
...
def _generate_vs(step: _core.VoltageScan) -> dict[str, str]:
    """Generate voltage scan dictionary."""
    return {
        "ctrl_type": "VS",
        "ctrl1_val": f"{step.scan_rate_mV_per_s}",
        "ctrl1_val_unit": "mV/s",
        "ctrl2_val": f"{step.start_voltage_V}",
        "ctrl2_val_unit": "V",
        "ctrl2_val_vs": "Ref",
        "ctrl3_val": f"{step.end_voltage_V}",
        "ctrl3_val_unit": "V",
        "ctrl3_val_vs": "Ref",
    }


def _generate_loop(step: _core.Loop) -> dict[str, str]:
